...
const matchPasswords = async (err, results, password, email, res) => {
  if (err) return res.status(401).json({ message: err });
    ...
    if (validPassword) return res.status(200).json({ email: email, id: results[0].id, messageType: "SUCCESS" });
...
export default async (req, res) => {
  ...
  try {
    ...
    return db.query(
      ...
      (err, results) => matchPasswords(err, results, password, email, res)
    );
  } catch (err) {
